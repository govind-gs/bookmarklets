...
async function storeTicketToClipboard(code, name) {
  var url = 'https://' + domain + '/browse/' + code;
  var data = CLIPBOARD_PREFIX + JSON.stringify({ code: code, name: name, url: url });
  await navigator.clipboard.writeText(data);
}

async function loadTicketFromClipboard() {
  var text = await navigator.clipboard.readText();
  if (!text || text.indexOf(CLIPBOARD_PREFIX) !== 0) return null;
  try { return JSON.parse(text.substring(CLIPBOARD_PREFIX.length)); } catch (e) { return null; }
}
...
async function handleJiraPage() {
  ...
  try {
    await storeTicketToClipboard(code, name);
    showSnackbar('Copied: ' + code + ' ' + name);
  } catch (e) {
...
async function handleAppianForm() {
  var ticket;
  try {
    ticket = await loadTicketFromClipboard();
  } catch (e) {
  ...
  if (!ticket) {
    showSnackbar('No ticket in clipboard. Click on a Jira ticket first.', true);
    return;
  }
